...
class Litbang extends CI_Controller {
	...
	public function index($level, $content = 'sosial-pemerintahan')
	{
    ...
    $data['isi'] = $this->load->view('pages/litbang/'.$content, $data, true);
	...
	public function detail($level, $content, $slug) {
    $this->session->set_userdata('level', $level);
    ...
    $data['levelLink'] = $level.'/';
    $data['header'] = $this->load->view('templates2/header', $data, true);
    $data['isi'] = $this->load->view('pages/litbang/'.$content.'-detail', $data, true);
    $this->load->view('templates2/index', $data);
